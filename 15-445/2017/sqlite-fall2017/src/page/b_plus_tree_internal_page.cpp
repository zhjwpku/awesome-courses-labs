...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::Init(page_id_t page_id,
  ...
  SetSize(0);
  SetMaxSize((PAGE_SIZE - sizeof(B_PLUS_TREE_INTERNAL_PAGE_TYPE)) / sizeof(MappingType));
...
B_PLUS_TREE_INTERNAL_PAGE_TYPE::Lookup(const KeyType &key,
  ...
  int idx = 1;
  for (; idx < GetSize(); idx++) {
    if (comparator(array[idx].first, key) == 1) { // find the first position > key
      break;
    }
  }
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::PopulateNewRoot(
  ...
  array[1].first = new_key;   // this is the newly created node
...
int B_PLUS_TREE_INTERNAL_PAGE_TYPE::InsertNodeAfter(
    ...
    const ValueType &new_value) {
  int idx = GetSize() - 1;
  for (; array[idx].second != old_value; idx --) {
    assert(idx > 0);
...
void B_PLUS_TREE_INTERNAL_PAGE_TYPE::CopyLastFrom(
  ...
  BPlusTreePage *node = reinterpret_cast<BPlusTreePage*>(page->GetData());
  node->SetParentPageId(GetPageId());
  assert(buffer_pool_manager->UnpinPage(pair.second, true));
}
